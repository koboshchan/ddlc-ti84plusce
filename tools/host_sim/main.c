...
static unsigned max_say_lines = 10000;
...
static minigame_pick_t minigames[16];
static unsigned minigame_count;
static unsigned minigame_pos;

static int16_t  custom_vars[VN_MAX_VARS];
static bool     custom_vars_set[VN_MAX_VARS];
static uint8_t *defaults_table;
static long     defaults_table_size;
...
static bool host_quit(void *ctx)
{
    ...
    return say_count > max_say_lines;
}
...
static uint8_t host_minigame(void *ctx, int16_t *s, int16_t *n, int16_t *y)
{
    (void)ctx;
    uint8_t winner = 0;
    int16_t s_val = 0, n_val = 0, y_val = 0;
    if (minigame_pos < minigame_count) {
        winner = minigames[minigame_pos].winner;
        s_val  = minigames[minigame_pos].s;
        n_val  = minigames[minigame_pos].n;
        y_val  = minigames[minigame_pos].y;
        minigame_pos++;
    }
    *s = s_val;
    *n = n_val;
    *y = y_val;
    if (opt_trace) {
        printf("[minigame winner=%u s=%d n=%d y=%d]\n", winner, s_val, n_val, y_val);
    }
    return winner;
}
...
static void parse_var(const char *arg)
{
    char *sep = strchr(arg, '=');
    if (!sep) sep = strchr(arg, ':');
    if (!sep) return;
    unsigned long slot = strtoul(arg, NULL, 10);
    long val = strtol(sep + 1, NULL, 10);
    if (slot < VN_MAX_VARS) {
        custom_vars[slot] = (int16_t)val;
        custom_vars_set[slot] = true;
    }
}

static void parse_minigame(const char *arg)
{
    const char *p = arg;
    while (*p != '\0' && minigame_count < sizeof(minigames) / sizeof(minigames[0])) {
        char *end;
        unsigned long w = strtoul(p, &end, 10);
        int16_t s = 0, n = 0, y = 0;
        if (*end == ',') {
            p = end + 1;
            s = (int16_t)strtol(p, &end, 10);
            if (*end == ',') {
                p = end + 1;
                n = (int16_t)strtol(p, &end, 10);
                if (*end == ',') {
                    p = end + 1;
                    y = (int16_t)strtol(p, &end, 10);
                }
            }
        }
        minigames[minigame_count].winner = (uint8_t)w;
        minigames[minigame_count].s = s;
        minigames[minigame_count].n = n;
        minigames[minigame_count].y = y;
        minigame_count++;
        p = (*end == ';') ? end + 1 : end;
        if (*p == ',') p++;
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--trace") == 0) {
            ...
            opt_quiet = true;
        } else if (strncmp(argv[i], "--max-lines=", 12) == 0) {
            max_say_lines = (unsigned)strtoul(argv[i] + 12, NULL, 0);
        } else if (strncmp(argv[i], "--seed=", 7) == 0) {
            opt_seed = (uint32_t)strtoul(argv[i] + 7, NULL, 0);
        } else if (strncmp(argv[i], "--choices=", 10) == 0) {
            parse_choices(argv[i] + 10);
        } else if (strncmp(argv[i], "--var=", 6) == 0) {
            parse_var(argv[i] + 6);
        } else if (strncmp(argv[i], "--minigame=", 11) == 0) {
            parse_minigame(argv[i] + 11);
        ...
        } else if (strncmp(argv[i], "--defaults=", 11) == 0) {
            FILE *f = fopen(argv[i] + 11, "rb");
            if (!f) {
                fprintf(stderr, "can't open %s\n", argv[i] + 11);
                return 2;
            }
            fseek(f, 0, SEEK_END);
            defaults_table_size = ftell(f);
            fseek(f, 0, SEEK_SET);
            defaults_table = malloc((size_t)defaults_table_size);
            if (!defaults_table || fread(defaults_table, 1, (size_t)defaults_table_size, f) !=
                                   (size_t)defaults_table_size) {
    ...
    if (defaults_table && defaults_table_size >= 2) {
        uint16_t count = (uint16_t)(defaults_table[0] | (defaults_table[1] << 8));
        const uint8_t *p = defaults_table + 2;
        uint16_t max_count = (uint16_t)((defaults_table_size - 2) / 3);
        if (count > max_count) count = max_count;
        for (uint16_t j = 0; j < count; j++) {
            uint8_t slot = p[0];
            int16_t val = (int16_t)(p[1] | (p[2] << 8));
            vm.vars[slot] = val;
            p += 3;
        }
    }
    for (int j = 0; j < VN_MAX_VARS; j++) {
        if (custom_vars_set[j]) {
            vm.vars[j] = custom_vars[j];
        }
    }
